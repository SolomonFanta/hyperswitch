    ...
    for diesel_models::business_profile::WebhookDetails
{
    fn foreign_from(item: api_models::admin::WebhookDetails) -> Self {
        Self {
            ...
            webhook_endpoint_id: Some(item.webhook_endpoint_id.unwrap_or_else(|| {
                // Generate a new webhook_endpoint_id if None, for example:
    ...
    for Vec<Option<diesel_models::business_profile::WebhookDetails>>
{
    fn foreign_from(item: Vec<Option<api_models::admin::WebhookDetails>>) -> Self {
        item.into_iter()
            .map(|webhook_detail| {
                webhook_detail.map(ForeignFrom::foreign_from) // Convert Option<api_models::admin::WebhookDetails> to Option<diesel_models::business_profile::WebhookDetails>
            })
            .collect()
    }
}
    ...
    for Vec<Option<api_models::admin::WebhookDetails>>
{
    fn foreign_from(item: Vec<Option<diesel_models::business_profile::WebhookDetails>>) -> Self {
        item.into_iter()
            .map(|webhook_detail| {
                webhook_detail.map(ForeignFrom::foreign_from) // Convert Option<diesel_models::business_profile::WebhookDetails> to Option<api_models::admin::WebhookDetails>
            })
            .collect()
    }
}
